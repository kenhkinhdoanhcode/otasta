...
def search(query, tld='com', lang='en', tbs='0', safe='off', num=10, start=0,
    ...
    while not stop or count < stop:
        ...
        try:
            text_searchs = []
            anchors = soup.find(id='search').findAll('a',{"class":"fuLhoc ZWRArf"})

            text_searchss = soup.find(id='search').findAll("div", {"class": "ezO2md"})
            for i in text_searchss:
                text = ""
                max_text = -1

                for j in i.findAll("span", {"class": "fYyStc"}):
                    if len(j.text)>max_text:
                        text = j.text
                        max_text = len(j.text)
                text_searchs.append(text)
            ...
            if gbar:
                gbar.clear()
            text_searchs = []
            anchors = soup.findAll('a',{"class":"fuLhoc ZWRArf"})

            text_searchss = soup.findAll("div", {"class": "ezO2md"})
            for i in text_searchss:
                text = ""
                max_text = -1
        ...
        for index,a in enumerate(anchors):
            ...
            # Yield the result.
            yield link,text_searchs[count%10]
